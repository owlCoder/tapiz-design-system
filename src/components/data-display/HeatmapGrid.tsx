  ...
  return (
    <div className={["grid gap-1", className].filter(Boolean).join(" ")} style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
      {cells.map((cell, index) => {
        ...
        return <div key={index} title={cell.title} className="aspect-square rounded-sm border border-(--tapiz-border-subtle)" style={{ background: `color-mix(in srgb, var(--tapiz-accent) ${Math.round(opacity * 100)}%, transparent)` }}>{cell.label ? <span className="sr-only">{cell.label}</span> : null}</div>;
      })}
    </div>
  );
}
