    ...
    <article className={`border border-(--tapiz-border-subtle) bg-(--tapiz-bg-surface) p-5 ${className}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-(--tapiz-text-primary)">{name}</h3>
            {description ? <p className="mt-1 text-sm leading-5 text-(--tapiz-text-muted)">{description}</p> : null}
          </div>
        </div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-(--tapiz-border-subtle) pt-4">
        <div className="font-mono text-xs text-(--tapiz-text-muted)">{lastSync ?? "No sync yet"}</div>
