...
export function SLAStatus({ label, value, target = 95, className = "" }: SLAStatusProps) {
  ...
  return (
    <div className={["rounded-lg border border-(--tapiz-border-subtle) bg-(--tapiz-bg-surface) p-4", className].filter(Boolean).join(" ")}>
      ...
      <div className="mt-3 h-2 overflow-hidden rounded-full border border-(--tapiz-border-subtle) bg-(--tapiz-bg-surface-muted)"><div className="h-full bg-(--tapiz-accent)" style={{ width: `${Math.max(0, Math.min(100, value))}%` }} /></div>
