...
export function PricingCard({ name, price, description, features = [], cta, highlighted = false, className = "" }: PricingCardProps) {
  return (
    <section className={`flex h-full flex-col border bg-(--tapiz-bg-surface) p-6 ${highlighted ? "border-2 border-(--tapiz-border-strong) shadow-(--tapiz-shadow-brutal-lg)" : "border-(--tapiz-border-subtle) shadow-(--tapiz-shadow-sm)"} ${className}`}>
