...
export function CommandMenu({ open, onClose, query = "", onQueryChange, groups, placeholder = "Search commands…", empty = "No commands found." }: CommandMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-start bg-(--tapiz-bg-overlay) px-4 pt-[12vh]" onClick={onClose}>
      <div className="mx-auto w-full max-w-2xl overflow-hidden rounded-lg border border-(--tapiz-border-strong) bg-(--tapiz-bg-surface) shadow-(--tapiz-shadow-lg)" onClick={(e) => e.stopPropagation()}>
          ...
          {groups.map((group, groupIndex) => (
            <div key={groupIndex} className="py-2">
              ...
              {group.items.map((item) => (
                <button key={item.id} type="button" disabled={item.disabled} onClick={item.onSelect} className="flex w-full items-center gap-3 rounded-md border border-transparent px-3 py-2 text-left hover:border-(--tapiz-border-subtle) hover:bg-(--tapiz-bg-surface-muted) disabled:opacity-40">
                  {item.icon ? <span className="grid size-8 place-items-center rounded-md border border-(--tapiz-border-subtle) text-(--tapiz-text-muted)">{item.icon}</span> : null}
